...
bool Map::CheckCollisionDest(Object* object) {
	for (auto& tile : m_tiles) {
		if (tile.m_isSolid) {
			if (AEC::collision::CheckCollisionAABB(*tile.GetDest(), *object->GetDest())) {
				return true;
			}
		}
	}
...
bool Map::CheckCollisionDest(SDL_Rect* dest) {
	for (auto& tile : m_tiles) {
		if (tile.m_isSolid) {
			if (AEC::collision::CheckCollisionAABB(*tile.GetDest(), *dest)) {
				return true;
			}
		}
	}
...
bool Map::CheckCollisionDest(Vec2 point) {
	for (auto& tile : m_tiles) {
		if (tile.m_isSolid) {
			if (AEC::collision::CheckCollisionPoint(*tile.GetDest(), point)) {
				return true;
			}
		}
	}
...
bool Map::CheckCollisionScreen(Object* object) {
	for (auto& tile : m_tiles) {
		if (tile.m_isSolid) {
			if (AEC::collision::CheckCollisionAABB(*tile.GetScreen(), *object->GetScreen())) {
				return true;
			}
		}
	}
	...
	for (auto& tile : m_tiles) {
		if (tile.m_isSolid) {
			if (destOrScreen) {
				if (AEC::collision::CheckCollisionAABB(*tile.GetDest(), *object->GetDest())) {
					if (XorY) {
						if (AEC::collision::CheckCollisionX(*tile.GetDest(), *object->GetDest()))
							return true;
					}
					else {
						if (AEC::collision::CheckCollisionY(*tile.GetDest(), *object->GetDest()))
							return true;
					}
				}
			}
				...
				if (AEC::collision::CheckCollisionAABB(*tile.GetScreen(), *object->GetScreen())) {
					if (XorY) {
						if (AEC::collision::CheckCollisionX(*tile.GetScreen(), *object->GetScreen()))
							return true;
					}
					else {
						if (AEC::collision::CheckCollisionY(*tile.GetScreen(), *object->GetScreen()))
							return true;
					}
				}
			}
